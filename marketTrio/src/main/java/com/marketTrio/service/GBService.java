...
//		MemberSession memberSession = (MemberSession) request.getAttribute("memberId");
    ...
    @Transactional
	public void participateCancel(String memberId) {
    	GBParticipantEntity gbPart = gbPartRepository.findByMemberId(memberId);
    	//Optional<GBParticipantEntity> gbp = gbPartRepository.findByMemberId(memberId);
		OptionEntity option = gbPart.getMyOption();
    	
		option.setRemainingQuantity(option.getRemainingQuantity() + gbPart.getMyOption().getQuantity());
        optionRepository.save(option);
    	
        gbPartRepository.save(gbPart);
	}
}